    ...
    if fss[7]: # /* если есть откр. ГВ из обратки */
        ...
        if hrc > 0.:
            ...
            if ferr:
                ...
                w_print(f"\t Остаток непогашенного напора равен {h_3mm - n_3mm * hrc} м.")
    ...
    if fss[0] == 1:
        if (otopl + vn + ggv) > 0.:
            ...
            if hrc >= H_DOPUST1:
                ...
                w_print(f" располагаемого напора ТП равного {hrc} м.")
                w_print(f" Расчет дроссельных органов выполняется одноступенчатым способом")

                return drvary1(drs, dr_out, rasp, pih, pihO, otopl, otn_fs, vn,
                    gvpr, gvsm, gvps, gvpw, gvop, gvoo, rez, fss,
...
#    /* Н */  case  2:
    ...
    if fss[6]: # /* если есть открытое ГВ из подачи */
        ...
        hrc = pih - drs['a12'] - drs['a10'] - dr_out['b13']
        if hrc > 0.:
            h_3mm = hrc
            n_3mm = 1
