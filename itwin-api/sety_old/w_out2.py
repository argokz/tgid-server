...
#        exit(0)
        ...
        q = f'insert into calculation (fileID, tn, date1, name, user_gid) $MS_SQL_OUTPUT$ values(?, ?, ?, ?, ?)'
#        c = db2.exec_insert(cursor, q, (fileID, config.args.Tn, dt, name))
        c = db2.exec_insert(cursor, q, (fileID, config.args.Tn, current_dateTime, name, config.args.user_gid))
