...
#    ist, heatSourceID = get_ist(externalCodeID)
    ...
    rs = read_gid.map_rs.get(externalCodeID)
    if rs is None:
        cprint(f'[red]Нет externalCodeID {externalCodeID} в {externalNodeName}[-]')
        exit(0)
...
#    tn_dt2_co = 0
    ...
    #            print(tP, t1)
            ...
            cprint('Какая-то странная ошибка, не должно быть такого', color='red')
            exit(0)
...
#        G_rez = (pt_gvop) * pr['rez'] * 1000 / t_ost / 100.
        G_rez = (pt_gvop + pt_gvoo) * pr['rez'] * 1000 / t_ost / 100.
 #       print(f'{G_rez}')
...
#        print('>>>>>>>>>>>>>', externalNodeName, pr_out['G_rez'], gvop + gvoo, ur_rez)
    ...
    if not no_kv and not kv is None:
        kvpot = kv.get('kvpot', 0)
        if kvpot > 0:
            pr_out['G_gvop'] *= kvpot
            pr_out['G_gvoo'] *= kvpot 

            pr_out['G_otoplz'] *= kvpot
            pr_out['G_otopln'] *= kvpot
            pr_out['G_ventil'] *= kvpot
            pr_out['G_kondiz'] *= kvpot
            pr_out['G_gvpr'] *= kvpot
            pr_out['G_gvsm'] *= kvpot
            pr_out['G_gvps'] *= kvpot
            pr_out['G_gvpw'] *= kvpot
        else:
            pr_out['G_gvop'] *= kv.get('gvop', 0)
            pr_out['G_gvoo'] *= kv.get('gvoo', 0) 

            pr_out['G_otoplz'] *= kv.get('otoplz', 0)
            pr_out['G_otopln'] *= kv.get('otopln', 0)
            pr_out['G_ventil'] *= kv.get('ventil', 0)
            pr_out['G_kondiz'] *= kv.get('kondiz', 0)
            pr_out['G_gvpr'] *= kv.get('gvz', 0)
            pr_out['G_gvsm'] *= kv.get('gvz', 0)
            pr_out['G_gvps'] *= kv.get('gvz', 0)
            pr_out['G_gvpw'] *= kv.get('gvz', 0)

    return pr_out

#-----------------------------------------------
def consumption_pr(G, pr, externalCodeID, externalNodeName, debug=False):
#    if debug:
#        print('1-----', pr)
...
#    print(pr_out['G_otoplz'])
    ...
    return gZ, gP, gO, Qz, Qp, Qo, pr_out
...
def consumption_po(G, po, externalCodeID, externalNodeName):
    tr = get_tr(po)
    ur = get_ur(po)
    kv = get_kv(po)
