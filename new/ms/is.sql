IF OBJECT_ID('dbo.is_view', 'V') IS NOT NULL
    DROP VIEW dbo.is_view;
GO

CREATE VIEW dbo.is_view AS
...
--    usO.ist AS [istO],                        -- Источник на обратке
   ...
   FROM nodes t
     ...
     JOIN heatsources ist ON ist.nodeid = t.id
