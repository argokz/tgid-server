IF OBJECT_ID('dbo.hs_view', 'V') IS NOT NULL
    DROP VIEW dbo.hs_view;
GO

CREATE VIEW dbo.hs_view AS
...
--    usO.ist AS [istO],                        -- Источник на обратке
   ...
   FROM nodes t
     ...
     JOIN pumpstations hs ON hs.nodeid = t.id
  ...
  WHERE t.removed = 0  AND t.internalnodeid IS NULL;
